...
import {CheckCircle2,CloudDownload,LoaderCircle,Monitor,RefreshCw,ShieldCheck} from 'lucide-react';
...
export default function SyncPage(){
  ...
  useEffect(()=>{load()},[]);
  const sync=async()=>{
    setSyncing(true);setMessage('请在弹出的 Windows 窗口中选择“是”…');
    try{
      const response=await fetch('/api/lcu/run',{method:'POST'});
      const result=await response.json();
      if(!response.ok)throw new Error(result.error);
      await load();setMessage('同步完成，最新对局已加入待复盘队列。');
    }catch(error:any){setMessage(error.message||'同步失败，请确认客户端已登录。')}
  ...
  return <div className="mx-auto max-w-5xl p-5 md:p-9">
    ...
    <div className="mt-8 grid gap-4 lg:grid-cols-[1fr_.7fr]">
      <section className="card p-6">
        <div className="flex items-start gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-mint/10 text-mint"><Monitor/></div><div><h2 className="text-lg font-bold">客户端已登录？直接同步</h2><p className="muted mt-1">点击后确认一次 Windows 权限提示，剩下的自动完成。</p></div></div>
        ...
        <p className="mt-4 text-xs leading-5 text-white/30">Windows 权限确认是为了读取由 WeGame 管理的客户端临时连接参数；应用不会获得或保存你的 QQ 密码。</p>
      </section>

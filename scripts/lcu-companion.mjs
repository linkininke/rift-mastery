...
function sync(){return new Promise((resolve,reject)=>{let output='';const child=spawn(process.execPath,[syncScript],{cwd:projectDir,windowsHide:true,env:{...process.env,RIFT_MASTERY_URL:process.env.RIFT_MASTERY_URL||config.RIFT_MASTERY_URL||webOrigin,RIFT_SYNC_TOKEN:process.env.RIFT_SYNC_TOKEN||config.RIFT_SYNC_TOKEN||'',RIFT_PROXY_URL:process.env.RIFT_PROXY_URL||config.RIFT_PROXY_URL||''}});child.stdout.on('data',data=>output+=data);child.stderr.on('data',data=>output+=data);child.on('error',reject);child.on('close',code=>code===0?resolve(output.trim()):reject(new Error(output.trim().split(/\r?\n/).filter(Boolean).pop()||'同步失败')))})}
